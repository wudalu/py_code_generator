"""
MCP (Model Context Protocol) Client 实现 - FastMCP v2 SSE 版本
连接到 CodeEmbedder MCP Server，使用代码向量化和搜索功能
使用 SSE 传输协议，遵循 MCP 最佳实践
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - [FASTMCP-SSE-CLIENT] - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)

class CodeEmbeddingClient:
    """代码嵌入 FastMCP SSE 客户端"""

    def __init__(self, server_url: str = "http://127.0.0.1:8000"):
        """初始化客户端

        Args:
            server_url: MCP 服务器 URL，默认为 http://127.0.0.1:8000
        """
    ...
    async def connect(self):
        ...
        # 创建 SSE 传输对象
        # 根据 FastMCP 文档，SSE 端点应该是 /sse
        sse_endpoint = f"{self.server_url}/sse"
        logger.info(f"使用 SSE 端点: {sse_endpoint}")
        transport = SSETransport(sse_endpoint)
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                try:
                    ...
                    return {"status": "success", "message": result[0].text}
            else:
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                try:
                    ...
                    return {"name": collection_name, "error": "无法解析资源内容"}
            else:
...
async def interactive_client(server_url: str = "http://127.0.0.1:8000"):
    """交互式客户端

    Args:
        server_url: 服务器 URL，默认为 http://127.0.0.1:8000
    """
    async with CodeEmbeddingClient(server_url) as client:
        print("客户端初始化完成！")
...
if __name__ == "__main__":
    ...
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description="FastMCP SSE 客户端")
    parser.add_argument("server_url", nargs="?", default="http://127.0.0.1:8000",
                        help="服务器 URL，默认为 http://127.0.0.1:8000")
